...
from src.model_evaluation import get_auc_scores, plot_decision_tree_surrogate, data_prep, train_predict_xgb, plot_compare_auc
from src.shap_plot import force_plot, summary_plot_numerical, summary_plot_categorical
from src.fpdp import get_fpdp_results, compute_vanilla_chi_squared, compute_conditional_chi_squared
...
CATEGORICAL_COLS = ['CreditHistory', 'EmploymentDuration', 'Housing', 
                    'Purpose', 'Savings']
NUMERICAL_COLS = data.loc[:, ~data.columns.isin(CATEGORICAL_COLS+TARGET_COLS)].columns.tolist()
...
class App:
    ...
    def create_main_pages(self):
        ...
        if self.option == "Model Performance Analysis":
            ...
            fig = plot_decision_tree_surrogate(X_check, y_check, model_features=model_features)
            st.pyplot(fig)
            ...
            custom_score, pre_score = get_auc_scores(test_data, X_train_prep, y_train, X_test_prep, y_test)
            ...
            fig3 = plot_decision_tree_surrogate(X_test_prep, y_prediction, model_features=model_features)
            st.pyplot(fig3)
        ...
        if self.option == "Fairness assessment":
            ...
            test_1 = get_fpdp_results(clf, dataset, model_features, column, title, group_column=y_true)
            st.pyplot(test_1)
            title2 = f"Conditional statistical parity for feature {column} depending on the 'group' column"
            test_2 = get_fpdp_results(clf, dataset, model_features, column, title2, group_column=dataset['Group'])
            st.pyplot(test_2)
            ...
            TO_DROP = st.multiselect("Variables to drop:", NUMERICAL_COLS+CATEGORICAL_COLS)
