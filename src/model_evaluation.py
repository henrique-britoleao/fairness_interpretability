...
def plot_decision_tree_surrogate(X_test_prep, y_pred, model_features):
    ...
    tree.plot_tree(clf, ax=ax, max_depth=2, impurity=False, fontsize=12, 
        filled=True, feature_names=model_features)
    return fig
...
def train_predict_xgb(X_train_prep, y_train, X_test_prep, y_test):
    ...
    param = {'lambda': 0.38873365526971265, 
    ...
    model.fit(X_train_prep, y_train, eval_set=[(X_test_prep, y_test)],early_stopping_rounds=100,verbose=False)
